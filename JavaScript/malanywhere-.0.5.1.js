...
function controller (request, sender, sendResponse) {
...
function findAnimeCreator(id, title, episodes) {
    ...
    function findAnime(data) {
        ...
        // If it wasn't found in the users list send the information we have to be displayed allows updating
            ...
            chrome.tabs.query({active: true, currentWindow: true}, function (tabs) {
                var activeTab = tabs[0];
                chrome.tabs.sendMessage(activeTab.id, {"message": "set status", "code": -1, "values": {
                    "series_title": title,
...
function insertError() {
    chrome.tabs.query({active: true, currentWindow: true}, function (tabs) {
        var activeTab = tabs[0];
        chrome.tabs.sendMessage(activeTab.id, {"message": "set status", "code": -2, "values": 0 });
    });
}
